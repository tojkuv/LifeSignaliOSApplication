...
/// Utilities for time-related operations in the app
enum TimeUtilities {
    // MARK: - Constants
    ...
    static func calculateExpirationDate(from checkInTime: Date, interval: TimeInterval) -> Date {
        // Ensure interval is at least the minimum
        let safeInterval = max(minimumInterval, interval)
    ...
    static func isNonResponsive(lastCheckIn: Date?, interval: TimeInterval) -> Bool {
    ...
    static func timeRemaining(lastCheckIn: Date, interval: TimeInterval) -> TimeInterval {
        let safeInterval = max(minimumInterval, interval)
        let expirationDate = lastCheckIn.addingTimeInterval(safeInterval)
    ...
    /// - Parameter interval: The time interval in seconds
    /// - Returns: A formatted string
    static func formatTimeIntervalWithFullUnits(_ interval: TimeInterval) -> String {
        let days = Int(interval / TimeUnits.day)
        let hours = Int((interval.truncatingRemainder(dividingBy: TimeUnits.day)) / TimeUnits.hour)
    ...
    static func createPastDate(hoursAgo: Double) -> Date {
