...
struct HomeViewComponent: View {
    ...
    @State private var shareImage: HomeComponentShareImage? = nil
    ...
    func shareQRCode() {
        if isImageReady, let image = qrCodeImage {
            shareImage = HomeComponentShareImage(image: image)
...
// MARK: - Instructions
struct HomeComponentInstructionsView: View {
...
struct HomeComponentShareImage: Identifiable {
    let id = UUID()
    let image: UIImage
}
