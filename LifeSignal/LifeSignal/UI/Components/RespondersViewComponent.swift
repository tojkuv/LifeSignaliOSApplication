...
struct RespondersViewComponent: View {
                ...
                LazyVStack(spacing: 12) {
                    if userViewModel.responders.isEmpty {
                        ...
                        ForEach(sortedResponders) { responder in
                            ResponderComponentCard(contact: responder, refreshID: refreshID)
                        }
                    }
                }
                .padding()
...
struct ResponderComponentCard: View {
    let contact: Contact
...
extension Array {
    func componentPartitioned(by belongsInFirstPartition: (Element) throws -> Bool) rethrows -> ([Element], [Element]) {
        var first: [Element] = []
        var second: [Element] = []
        for element in self {
