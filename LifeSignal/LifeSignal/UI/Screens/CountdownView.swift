...
struct CountdownView: View {
            ...
            VStack(spacing: 24) {
                ...
                ZStack {
                    ...
                    VStack(spacing: 8) {
                        ...
                        Text(userProfileViewModel.timeUntilNextCheckIn)
                ...
                VStack(spacing: 16) {
                    ...
                    HStack {
                        Text("Last checked in:")
                            .foregroundColor(.primary)
                        Spacer()
                        Text(formatDate(userProfileViewModel.lastCheckedIn))
                            .foregroundColor(.secondary)
                    }
                    .padding(.horizontal)
                    ...
                    HStack {
                        Text("Next check-in due:")
                            .foregroundColor(.primary)
                        Spacer()
                        Text(formatDate(userProfileViewModel.checkInExpiration))
                            .foregroundColor(.secondary)
                    }
                    .padding(.horizontal)
                }
    ...
    private func calculateProgress() -> CGFloat {
        let totalInterval = userProfileViewModel.checkInInterval
        let elapsed = Date().timeIntervalSince(userProfileViewModel.lastCheckedIn)
